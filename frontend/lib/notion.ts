...
function mapNotionPageToBrand(page: PageObjectResponse): Brand {
  ...
  return {
    ...
    description: getPlainText(
      getProperty(props, "Descript", "Description", "description", "설명"),
    ),
    logoUrl:
      getUrl(
        getProperty(props, "Logo URL", "LogoUrl", "logoUrl", "logo_url"),
    ...
    capsuleCount:
      getRollupNumber(
        getProperty(props, "Capsule Count", "capsuleCount", "CapsuleCount"),
      ) ??
      getNumber(
        getProperty(props, "Capsule Count", "capsuleCount", "CapsuleCount"),
      ) ??
...
function mapNotionPageToCapsule(
  ...
  const intensityRaw = getNumber(
    getProperty(props, "Intensity", "intensity", "강도"),
  );
  const intensity =
  ...
  const brandIds = getRelation(
    getProperty(props, "Brand", "brand", "BrandId", "brandId"),
  );
  ...
  return {
    ...
    name: getPlainText(getProperty(props, "Name", "name", "이름")),
    description: getPlainText(
      getProperty(props, "Description", "description", "설명"),
    ),
    imageUrl:
      getFiles(
        getProperty(
    ...
    isDiscontinued: getCheckbox(
      getProperty(props, "IsDiscontinued", "isDiscontinued", "Discontinued"),
    ),
    // Notion 프로퍼티명 'AverageRating'은 쿠팡 연동 평점을 의미
    coupangRating: getNumber(
      getProperty(props, "AverageRating", "averageRating", "Average Rating"),
    ),
    reviewCount:
      getNumber(
        getProperty(props, "ReviewCount", "reviewCount", "Review Count"),
      ) ?? 0,
  };
}
...
export async function getBrands(): Promise<Brand[]> {
  try {
    ...
    return response.results.filter(isFullPage).map(mapNotionPageToBrand);
  } catch (error) {
...
export async function getCapsulesByBrandId(
  brandId: string,
): Promise<Capsule[]> {
  try {
...
export async function getCapsuleBySlug(slug: string): Promise<Capsule | null> {
  try {
    ...
    for (const page of pages) {
      ...
      const capsule = mapNotionPageToCapsule(
        page,
        brand?.name ?? "",
        brand?.slug ?? "",
      );
